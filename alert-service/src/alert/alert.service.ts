...
import { AlertPriority } from 'shared';
  ...
  async generateAlertsForBranch(branchId: number): Promise<void> {
    ...
    try {
      const response = await firstValueFrom(
        this.httpService.get(`${this.purchaseServiceUrl}/purchases/inventory/${branchId}`)
      );
      inventoryData = response.data.data || response.data;
    } catch (error) {
      console.error('Failed to fetch inventory data from purchase service:', error);
      return;
    }
